...
export const AuthProvider = ({ children }) => {
      ...
      .then((res) => {
        ...
        if (res !== null)
          if (Cookies.get('restai_token')) {
            setUser({ username: username, expires: 86400, created: Math.floor(Date.now() / 1000), admin: res.is_admin });
          } else {
            setUser({ username: username, basicAuth: basicAuth, expires: 86400, created: Math.floor(Date.now() / 1000), admin: res.is_admin });
          }
      })
  };
  const logout = () => {
  ...
  const checkAuth = () => {
    if (Cookies.get('restai_token') !== undefined && user == null) {
      ...
      login(user.username);
      return false;
    } else if(Cookies.get('restai_token') === undefined && user !== null && user !== undefined && user.basicAuth === undefined) {
      logout();
      return false;
    }
    if (user !== null) {
